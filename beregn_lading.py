...
def beregn():
    ...
    for _, rad in gruppert.iterrows():
        ...
        if beboer_nr not in beboere:
            advarsler.add(f"Ukjent ladepunkt {beboer_nr} (CloudCharge uttak {beboer_nr - 100}) — mangler i beboerregisteret")
            continue
        ...
        if pris_nøkkel not in priser:
            advarsler.add(f"Mangler strømpris for {MÅNEDER[måned]} {år} — legg inn i Excel-filen")
            pris_ore = None
            kostnad = None
        else:
    ...
    total_kwh = sum(r["Forbruk (kWh)"] for r in resultater)
    total_kr = sum(r["Strømkost (kr)"] for r in resultater if r["Strømkost (kr)"])
...
def skriv_excel(resultater, filsti):
    ...
    annenhver_fill = PatternFill(start_color="DCE6F1", end_color="DCE6F1", fill_type="solid")

    for rad_nr, rad in enumerate(resultater, 2):
        fyll = annenhver_fill if rad_nr % 2 == 0 else None

        def celle(col, verdi, tallformat=None):
        ...
        celle(1, rad["Ladepunkt"])
        celle(2, rad["Navn"])
        celle(3, rad["Leilighet"])
        celle(4, rad["Måned"])
        celle(5, rad["Forbruk (kWh)"], "#,##0.00")
        celle(6, rad["Strømpris (øre/kWh)"], "#,##0.00")
        celle(7, rad["Strømkost (kr)"], "#,##0.00")
